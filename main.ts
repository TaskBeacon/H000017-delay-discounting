...
export async function run(root: HTMLElement): Promise<void> {
  ...
  const conditionGenerationConfig = ((parsed.raw.condition_generation as ConditionGenerationConfig) ??
  ...
  await mountTaskApp({
    root,
    task_id: "H000017-delay-discounting",
    task_name: "Delay Discounting Task",
    task_description: "HTML preview aligned to local psyflow Delay Discounting procedure and parameters.",
    settings,
    subInfo,
    stimBank,
